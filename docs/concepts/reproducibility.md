...
If software is reproducible, rebuilding it should always produce the same result. [Hash functions](https://en.wikipedia.org/wiki/Hash_function) can then be used to verify that the output has not changed.
...
Without reproducibility, major supply-chain compromises like [SolarWinds](https://en.wikipedia.org/wiki/2020_United_States_federal_government_data_breach) can go undetected.
...
The security engineers behind Caution developed the [StageX](https://stagex.tools) Linux distribution to support this approach.
